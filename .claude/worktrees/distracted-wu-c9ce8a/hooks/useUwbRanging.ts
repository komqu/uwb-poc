  ...
  const startAsControlee = useCallback(async (codeInput: string) => {
    ...
    const found = await new Promise<Device | null>((resolve) => {
      ...
      ble.startDeviceScan([SERVICE_UUID], null, (err, device) => {
        ...
        // The controller embeds the pairing code as manufacturer data.
        // Layout: 2-byte little-endian company ID (0xFFFF) + UTF-8 pairing code.
        if (device?.manufacturerData) {
          const bytes = Buffer.from(device.manufacturerData, 'base64');
          const code = bytes.slice(2).toString('utf8');
          if (code === target) {
            if (scanTimeoutRef.current) clearTimeout(scanTimeoutRef.current);
            ble.stopDeviceScan();
            resolve(device);
